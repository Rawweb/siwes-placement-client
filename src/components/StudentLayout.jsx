...
// The student navigation items, mapped into the sidebar and bottom nav.
const navItems = [
  { to: '/student/browse', label: 'Browse Opportunities', short: 'Browse', icon: '⌕' },
  { to: '/student/applications', label: 'My Applications', short: 'Applications', icon: '▤' },
  { to: '/student/profile', label: 'My Profile', short: 'Profile', icon: '◉' },
];
...
export default function StudentLayout() {
  ...
  // Log out and send the user back to the landing page.
  const handleLogout = () => {
    logout();
    navigate('/');
  };

  // Build the initials shown in the avatar circle from the user's name.
    ...
    <div className='min-h-screen md:grid md:grid-cols-[230px_1fr] bg-[var(--color-paper)]'>
      ...
      <div className='flex flex-col min-w-0'>
        ...
        <header className='flex items-center gap-3 bg-white border-b border-[var(--color-line)] px-4 md:px-6 py-3'>
          {/* small logo shown only on mobile, since the sidebar is hidden there */}
        ...
        {/* The current student page renders here */}
